...
function networkByName(net, offset) {
...
export class EventID {
    ...
    static createRandom(networkID, networkIDOffset, optionalParams = {}) {
        const { separatorKey = 'model', separatorValue = 'kh4q0ozorrgaq2mezktnrmdwleo1d', controller = 'did:key:z6MkgSV3tAuw7gUWqKCUY7ae6uWNxqYgdwPhUJbJhF9EFXm9', } = optionalParams;
        const init = randomCID();
        const eventHeight = Math.floor(Math.random() * 1000000);
        const event = randomCID();
        return this.create(networkID, networkIDOffset, separatorKey, separatorValue, controller, init, eventHeight, event);
    }
    static create(networkID, networkIDOffset, separatorKey, separatorValue, controller, init, eventHeight, event) {
        const networkIDInt = typeof networkID === 'string' ? networkByName(networkID, networkIDOffset) : networkID;
        const separatorKeyBytes = typeof separatorKey === 'string' ? u8a.fromString(separatorKey) : separatorKey;
        const separatorValueBytes = typeof separatorValue === 'string' ? u8a.fromString(separatorValue) : separatorValue;
